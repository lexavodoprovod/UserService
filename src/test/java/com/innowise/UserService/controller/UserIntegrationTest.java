...
import com.innowise.UserService.model.service.impl.PaymentCardServiceImpl;
...
public class UserIntegrationTest {
    ...
    public void setUp() {
        ...
        // Регистрируем модуль для работы с LocalDate, LocalDateTime и т.д.
        ...
        paymentCardDto = PaymentCardDto.builder()
                .userId(1L)
                .number("9112667744889922")
                .holder("NIKITA HOLOLEENKO")
                .expirationDate(LocalDate.of(2028, 1, 1))
                .active(true)
                .build();
    }
    ...
    static void configureProperties(DynamicPropertyRegistry registry) {
        // Подставляем URL, который сгенерировал Testcontainers (порт будет случайным, чтобы не было конфликтов)
    ...
    @Nested
    @DisplayName("Get User With Cards Integration Tests")
    class GetUserWithCardsRequest {

        @Test
        @DisplayName("Should return user with cards successfully")
        void shouldReturnUserWithCardsTest() throws Exception{
            ...
            PaymentCard card = PaymentCard.builder()
                    ...
                    .user(savedUser)
                    .expirationDate(LocalDate.of(2030, 9, 25))
                    .build();
            ...
            mockMvc.perform(MockMvcRequestBuilders
                            .get("/users/user-with-cards/%s".formatted(userId))
                    .accept(MediaType.APPLICATION_JSON))
                    .andExpect(MockMvcResultMatchers.status().isOk());

            assertEquals(1, paymentCardDao.countPaymentCardByUserId(userId));
        ...
        void shouldReturn404WhenUserNotFound() throws Exception {
            ...
            mockMvc.perform(MockMvcRequestBuilders
                            .get("/users/user-with-cards/" + nonExistentId)
                            .accept(MediaType.APPLICATION_JSON))
                    .andExpect(MockMvcResultMatchers.status().isNotFound());
        }

        @Test
        @DisplayName("Should return user with empty cards list")
        void shouldReturnUserWithEmptyCards() throws Exception {
            User user = User.builder()
                    .name("Roma")
                    .surname("Dovidenko")
                    .email("roma@example.com")
        ...
        @Test
        @DisplayName("Should return user with multiple cards")
        void shouldReturnUserWithMultipleCards() throws Exception {
            User user = User.builder()
                    .name("Roma")
                    .surname("Dovidenko")
                    .email("roma@example.com")
